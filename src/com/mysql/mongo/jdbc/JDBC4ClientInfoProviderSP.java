...
public class JDBC4ClientInfoProviderSP implements JDBC4ClientInfoProvider {
    ...
    public synchronized void initialize(java.sql.Connection conn, Properties configurationProps) throws SQLException {
        ...
        this.setClientInfoSp = ((com.mysql.mongo.jdbc.Connection) conn).clientPrepareStatement(
                "CALL " + identifierQuote + catalog + identifierQuote + "." + identifierQuote + setClientInfoSpName + identifierQuote + "(?, ?)");

        this.getClientInfoSp = ((com.mysql.mongo.jdbc.Connection) conn).clientPrepareStatement(
                "CALL" + identifierQuote + catalog + identifierQuote + "." + identifierQuote + getClientInfoSpName + identifierQuote + "(?)");

        this.getClientInfoBulkSp = ((com.mysql.mongo.jdbc.Connection) conn).clientPrepareStatement(
