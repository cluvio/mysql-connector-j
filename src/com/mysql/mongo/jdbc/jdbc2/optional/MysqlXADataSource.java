...
public class MysqlXADataSource extends MysqlDataSource implements javax.sql.XADataSource {
    ...
    private XAConnection wrapConnection(Connection conn) throws SQLException {
        if (getPinGlobalTxToPhysicalConnection() || ((com.mysql.mongo.jdbc.Connection) conn).getPinGlobalTxToPhysicalConnection()) {
            return SuspendableXAConnection.getInstance((com.mysql.mongo.jdbc.Connection) conn);
        }

        return MysqlXAConnection.getInstance((com.mysql.mongo.jdbc.Connection) conn, getLogXaCommands());
    }
}
